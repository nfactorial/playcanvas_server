...
class GameServer {
    /**
     *
     * @param logger The object to be used for logging, if not specified the default logger will be used.
     */
    constructor(logger) {
        ...
        this.authService = new AuthService(this.log);

        if (process.env.FACEBOOK_APP_ID) {
            this.authService.configure(this.expressServer);
        } else {
            this.log.info('Facebook App ID not found, skipping Facebook authentication service.');
        }
    }
    ...
    run(port) {
        ...
        this.app.on('connection', (socket) => {
            if (this.connections.length < this.maximumConnections) {
                ...
                this.log.info('Connection denied, too many users');
...
*/
    ...
    removeConnection(connection) {
        ...
        if (-1 === index) {
            this.log.info('GameServer.removeConnection - Specified connection could not be found.');
        } else {
